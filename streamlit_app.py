...
# Google Drive 파일 ID
file_id = '1W_iO7tQ-qJxW-Gi0JoQsp27OjEx-I_t_'
...
def display_right_content(labels):
    ...
    for i, label in enumerate(labels[:3]):
        with cols[i]:
            st.image(f"https://i.ibb.co/3dXN2Rc/image.webp?text={label}", caption=f"이미지: {label}", use_column_width=True)

    # 2nd Row - YouTube Videos based on labels
    for i, label in enumerate(labels[:3]):
        with cols[i]:
            st.video("https://www.youtube.com/watch?v=ceGXF7zTk1Y", start_time=0)
